...
static PLP_L1_DATA       unsigned char loc[MAX_BUFFER_SIZE];
static PLP_FC_GLOBAL_DATA unsigned char ext[MAX_BUFFER_SIZE];
  ...
  if (get_core_id() == 0){
    ...
    for ( int i = 4; i < 128; i=i+4) { // size
      for ( int j = 0; j < 16; j=j+4) { // base address
    	error_count += testMCHAN(i, TX, INC, LIN, 1, 0, 0, EXT_DATA_ADDR + j, TCDM_DATA_ADDR, 0, 0);
      }
    }
    
    for ( int i = 4; i < 128; i=i+4) { //size
      for ( int j = 0; j < 16; j=j+4 ) { //base address
	error_count += testMCHAN(i, RX, INC, LIN, 1, 0, 0, EXT_DATA_ADDR + j, TCDM_DATA_ADDR, 0, 0);
      }
    }
