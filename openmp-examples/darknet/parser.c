...
LAYER_TYPE string_to_layer_type(char *type) {
  ...
  if (strcmp(type, "[conn]") == 0 || strcmp(type, "[connected]") == 0) return CONNECTED;
  ...
  if (strcmp(type, "[lrn]") == 0 || strcmp(type, "[normalization]") == 0) return NORMALIZATION;
...
convolutional_layer parse_convolutional(list *options, size_params params) {
  ...
  convolutional_layer layer =
      make_convolutional_layer(batch, h, w, c, n, groups, size, stride, padding, activation,
...
layer parse_crnn(list *options, size_params params) {
  ...
  layer l = make_crnn_layer(params.batch, params.w, params.h, params.c, hidden_filters,
                            output_filters, params.time_steps, activation, batch_normalize);
...
layer parse_rnn(list *options, size_params params) {
  ...
  layer l = make_rnn_layer(params.batch, params.inputs, output, params.time_steps, activation,
                           batch_normalize, params.net->adam);
...
layer parse_connected(list *options, size_params params) {
  ...
  layer l = make_connected_layer(params.batch, params.inputs, output, activation, batch_normalize,
                                 params.net->adam);
  return l;
}
...
layer parse_normalization(list *options, size_params params) {
  ...
  layer l = make_normalization_layer(params.batch, params.w, params.h, params.c, size, alpha, beta,
                                     kappa);
  return l;
}
...
layer parse_shortcut(list *options, size_params params, network *net) {
  ...
  layer s = make_shortcut_layer(batch, index, params.w, params.h, params.c, from.out_w, from.out_h,
                                from.out_c);
...
void save_weights(network *net, char *filename) { save_weights_upto(net, filename, net->n); }
...
void load_connected_weights(layer l, FILE *fp, int transpose) {
  ...
  // variance_array(l.biases, l.outputs)); printf("Weights: %f mean %f variance\n",
  ...
  if (l.batch_normalize && (!l.dontloadscales)) {
    ...
    // variance_array(l.scales, l.outputs)); printf("rolling_mean: %f mean %f variance\n",
...
void load_weights(network *net, char *filename) { load_weights_upto(net, filename, 0, net->n); }
