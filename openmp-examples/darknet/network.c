...
void forward_network(network *netp) {
#ifdef TIMELAYERS
  struct timespec start_timer, stop_timer;
  double accum;
#endif  // TIMELAYERS
#ifdef GPU
  if (netp->gpu_index >= 0) {
  ...
  for (i = 0; i < net.n; ++i) {
    ...
    printf("Layer %d, %lf\n", i,
           (stop_timer.tv_sec - start_timer.tv_sec) +
               (stop_timer.tv_nsec - start_timer.tv_nsec) / CLOCK_PRECISION);
#endif  // TIMELAYERS
  }
...
int get_predicted_class_network(network *net) { return max_index(net->output, net->outputs); }
...
void set_batch_network(network *net, int b) {
  ...
  for (i = 0; i < net->n; ++i) {
    ...
    if (net->layers[i].type == DECONVOLUTIONAL) {
      layer *l = net->layers + i;
      cudnnSetTensor4dDescriptor(l->dstTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1, l->out_c,
                                 l->out_h, l->out_w);
      cudnnSetTensor4dDescriptor(l->normTensorDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 1,
                                 l->out_c, 1, 1);
    }
#endif
  }
