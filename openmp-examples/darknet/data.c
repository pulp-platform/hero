...
#include "utils.h"
#ifndef __PULP__
#warning Compiling non-PULP
#include <pthread.h>
#else
#warning Compiling RISC-V
...
/*
__host char**get_random_paths_indexes(__host char**paths, int n, int m, int *indexes)
{
...
matrix load_image_augment_paths(__host char **paths, int n, int min, int max, int size, float angle,
...
void fill_truth_swag(char *path, float *truth, int classes, int flip, float dx, float dy, float sx,
                     float sy) {
  char labelpath[4096];
...
void fill_truth_region(char *path, float *truth, int classes, int num_boxes, int flip, float dx,
...
void fill_truth_iseg(char *path, int num_boxes, float *truth, int classes, int w, int h,
                     augment_args aug, int flip, int mw, int mh) {
  char labelpath[4096];
  ...
  while ((fscanf(file, "%d %s", &id, buff) == 2) && i < num_boxes) {
    ...
    load_rle(part, rle, n);
    image sized =
        rotate_crop_image(part, aug.rad, aug.scale, aug.w, aug.h, aug.dx, aug.dy, aug.aspect);
...
void fill_truth_mask(char *path, int num_boxes, float *truth, int classes, int w, int h,
                     augment_args aug, int flip, int mw, int mh) {
  char labelpath[4096];
  find_replace(path, "images", "mask", labelpath);
  find_replace(labelpath, "JPEGImages", "mask", labelpath);
...
__host char **get_labels(char *filename) {
  ...
  __host char **labels = (__host char **)list_to_array(plist);
  free_list(plist);
  return labels;
}
...
data load_data_seg(int n, __host char **paths, int m, int w, int h, int classes, int min, int max,
  ...
  for (i = 0; i < n; ++i) {
    ...
    image sized_m = rotate_crop_image(mask, a.rad, a.scale / div, a.w / div, a.h / div, a.dx / div,
                                      a.dy / div, a.aspect);
...
data load_data_iseg(int n, __host char **paths, int m, int w, int h, int classes, int boxes,
                    int div, int min, int max, float angle, float aspect, float hue,
  ...
  for (i = 0; i < n; ++i) {
    ...
    fill_truth_iseg(random_paths[i], boxes, d.y.vals[i], classes, orig.w, orig.h, a, flip, w / div,
                    h / div);
...
data load_data_mask(int n, __host char **paths, int m, int w, int h, int classes, int boxes,
                    int coords, int min, int max, float angle, float aspect, float hue,
                    float saturation, float exposure) {
  __host char **random_paths = get_random_paths(paths, n, m);
  int i;
  data d = {0};
...
data load_data_region(int n, __host char **paths, int m, int w, int h, int size, int classes,
...
data load_data_detection(int n, __host char **paths, int m, int w, int h, int boxes, int classes,
                         float jitter, float hue, float saturation, float exposure) {
  __host char **random_paths = get_random_paths(paths, n, m);
  int i;
  data d = {0};
  ...
  for (i = 0; i < n; ++i) {
    ...
    fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, flip, -dx / w, -dy / h,
                         nw / w, nh / h);
...
void *load_thread(void *ptr) {
  ...
  if (a.type == OLD_CLASSIFICATION_DATA) {
    ...
    *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy, a.min, a.max,
    ...
    *a.d = load_data_iseg(a.n, a.paths, a.m, a.w, a.h, a.classes, a.num_boxes, a.scale, a.min,
                          a.max, a.angle, a.aspect, a.hue, a.saturation, a.exposure);
  } else if (a.type == INSTANCE_DATA) {
    *a.d = load_data_mask(a.n, a.paths, a.m, a.w, a.h, a.classes, a.num_boxes, a.coords, a.min,
                          a.max, a.angle, a.aspect, a.hue, a.saturation, a.exposure);
    ...
    *a.d = load_data_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue,
                            a.saturation, a.exposure);
  } else if (a.type == DETECTION_DATA) {
    *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue,
                               a.saturation, a.exposure);
    ...
    *a.d = load_data_tag(a.paths, a.n, a.m, a.classes, a.min, a.max, a.size, a.angle, a.aspect,
...
/*
   data load_data_study(char **paths, int n, int m, char **labels, int k, int min, int max, int
   size, float angle, float aspect, float hue, float saturation, float exposure)
   {
   data d = {0};
   d.indexes = calloc(n, sizeof(int));
   if(m) paths = get_random_paths_indexes(paths, n, m, d.indexes);
   d.shallow = 0;
   d.X = load_image_augment_paths(paths, n, min, max, size, angle, aspect, hue, saturation,
   exposure); d.y = load_labels_paths(paths, n, labels, k); if(m) free(paths); return d;
   }
 */
...
data load_data_regression(__host char **paths, int n, int m, int k, int min, int max, int size,
                          float angle, float aspect, float hue, float saturation, float exposure) {
  if (m) paths = get_random_paths(paths, n, m);
  data d = {0};
  d.shallow = 0;
...
data load_data_augment(__host char **paths, int n, int m, __host char **labels, int k,
                       tree *hierarchy, int min, int max, int size, float angle, float aspect,
...
data load_data_tag(__host char **paths, int n, int m, int k, int min, int max, int size,
                   float angle, float aspect, float hue, float saturation, float exposure) {
  if (m) paths = get_random_paths(paths, n, m);
  data d = {0};
  d.w = size;
