...
gomp_offload_manager ( )
{
    ...
    while(1) {
        ...
        if (DEBUG_LEVEL_OFFLOAD_MANAGER > 0)
          rt_debug("nbOffloadRabMissHandlers %d/%d\n", nbOffloadRabMissHandlers, rt_nb_active_pe());

        // (3c) Spawning nbOffloadRabMissHandlers
        unsigned mhCoreMask = 0;
        nbOffloadRabMissHandlers = nbOffloadRabMissHandlers < rt_nb_active_pe()-1 ? nbOffloadRabMissHandlers : rt_nb_active_pe()-1;
        if(nbOffloadRabMissHandlers) {
            offload_rab_miss_sync = 0x0U;
            for(int pid = rt_nb_active_pe()-1, i = nbOffloadRabMissHandlers; i > 0; i--, pid--) {
                if (DEBUG_LEVEL_OFFLOAD_MANAGER > 0)
                    rt_debug("enabling RAB miss handler on %d\n", pid);
                mhCoreMask |= (1 << pid);
            }
        }
        omp_getData()->maxThreads = rt_nb_active_pe() - nbOffloadRabMissHandlers;
        omp_getData()->numThreads = rt_nb_active_pe() - nbOffloadRabMissHandlers;
