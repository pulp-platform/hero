...
make tc-llvm
```
Note that the LLVM OpenMP target offloading plugin for PULP (i.e., `libomptarget.rtl.pulp.so`) is *not* compiled as part of LLVM.  Instead, it is compiled as package in the Host SDK, because it must be compiled for a specific Host architecture.  You can thus safely ignore LLVM compilation messages such as `LIBOMPTARGET: Not building PULP offloading plugin: PULP SDK not found (HERO_PULP_INC_DIR to the correct path`.
